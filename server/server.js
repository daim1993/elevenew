...
async function pgInit(){
  ...
  for(const row of r.rows){
    if(row.k === "db"){ try{ db = Object.assign(db, JSON.parse(row.v)); hadDb = true; }catch(e){ console.error("pg db parse", e.message); } }
    else if(row.k.indexOf("portal:") === 0){ try{ pCache.set(row.k.slice(7), JSON.parse(row.v)); }catch(e){} }
  }
...
app.use(cors(ORIGINS.length?{ origin:(origin,cb)=>cb(null, !origin||ORIGINS.includes(origin)) }:undefined));
...
function savePortal(uidStr,doc,immediate){
  doc.updatedAt=Date.now(); pCache.set(uidStr,doc); pNotify(uidStr,doc); insDirty();
  clearTimeout(pTimers.get(uidStr));
  const write=()=>{ try{ fs.writeFileSync(pFile(uidStr)+".tmp",JSON.stringify(doc)); fs.renameSync(pFile(uidStr)+".tmp",pFile(uidStr)); }catch(e){ console.error("portal persist",e.message); } pgMark("portal:"+uidStr); };
...
app.post("/api/admin/users",(req,res)=>{ const actor=can(req,res,"accounts","create"); if(!actor) return;
  ...
  const u={ id:uid(), email:email.toLowerCase(), salt, pass:hash(password,salt), role:ROLE_OK.indexOf(role)>=0?role:"user", plan:"free", createdAt:Date.now() };
...
setInterval(()=>{ if(!insSubs.size) return; const lc=JSON.stringify(liveCounts()); if(lc!==lastLive){ lastLive=lc; insEmit(); } }, 5000); // presence join/leave → push within 5s
...
function tailEvents(n){ try{ const lines=fs.readFileSync(LOG,"utf8").trim().split("\n"); const out=[];
